...
/* Filtrado */
$where = "archivado = 'no' and rol = 'Vendedor'";
...
if ($num_rows > 0) {
    while ($row = $resultado->fetch_assoc()) {
        ...
        $output['data'] .= '<td>' . $row['nombre'] . $espacio . $row['aPaterno'] . $espacio . $row['aMaterno'] . '</td>';
         ...
         $output['data'] .= '<td class="nombre-usuario">' . $row['telefono'] . '</td>';

        $output['data'] .= '<td class="text-center">' . $row['fecha_ingreso'] . '</td>';

        $output['data'] .= '<td class="align-middle text-center text-sm"><span class="text-secondary text-xs font-weight-bold"><a class="btn btn-primary" href="pagar-comisiones.php?id_usuario=' . $row['id_usuario'] . '">Gestionar comisiones</a></span></td>';

        $output['data'] .= '<td class="align-middle text-center text-sm"><span class="text-secondary text-xs font-weight-bold"><a class="btn boton-secundario" href="historial-empleado.php?id_usuario=' . $row['id_usuario'] . '">Ver historial</a></span></td>';

        $output['data'] .= '<td class="align-middle text-center text-sm"><span class="text-secondary text-xs font-weight-bold"><a class="btn boton-secundario" href="updates/archivar-vendedores.php?id_usuario=' . $row['id_usuario'] . '">Archivar</a></span></td>'; 
