...
$columns = ['id_paciente', 'nombre', 'aPaterno', 'aMaterno', 'edad', 'fechaIngreso'];
...
if ($num_rows > 0) {
    while ($row = $resultado->fetch_assoc()) {
        ...
        $output['data'] .= '<td class="nombre-alumno">' . $row['edad'] . '</td>';
        $output['data'] .= '<td class="nombre-alumno text-center">' . $row['fechaIngreso'] . '</td>';
       

        // Asegurarse de que "id_alumno" esté definido en la fila antes de intentar acceder
       
        $output['data'] .= '<td class="align-middle text-center text-sm"><span class="text-secondary text-xs font-weight-bold"><a class="btn boton-secundario" href="perfil-apoyo.php?id_paciente=' . $row['id_paciente'] . '">Ver perfil</a></span></td>';
