...
if ($num_rows > 0) {
    while ($row = $resultado->fetch_assoc()) {
        ...
        $output['data'] .= '<td class="align-middle text-center text-sm"><span class="text-secondary text-xs font-weight-bold"><a class="btn boton-secundario" href="updates/archivar-paciente.php?id_paciente=' . (isset($row['id_paciente']) ? $row['id_paciente'] : '') . '">Archivar paciente</a></span></td>';
        $output['data'] .= '</tr>';
    }
} else {
    $output['data'] .= '<tr>';
